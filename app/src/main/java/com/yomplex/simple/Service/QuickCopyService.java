...
public class QuickCopyService extends JobIntentService {

    public static final String URL = "url";
    /**
     * Unique job ID for this service.
     */
    static final int DOWNLOAD_JOB_ID = 2222;
    /**
     * Actions download
     */
    ...
    public static void enqueueWork(final Context context) {
        ...
        QuickCopyService.context1 = context;
        ...
        try{
            ...
            for(int i = 0;i<booksCountmodel.size();i++){
                ...
                int count = dataBase.getQuickBooksCount(booksCount.getId());
                if (count == 0) {
                    Log.e("dash board","booksJsonString...count......."+count);
                    dataBase.insertQuickBooks(booksCount);
                }
            }
        }catch (Exception e){
        ...
        Intent intent = new Intent(context1, QuickCopyService.class);
        /*intent.putExtra(URL, url);
        intent.putExtra("version", version);
        intent.putExtra("title", title);
        intent.putExtra("category", category);*/
        intent.setAction(ACTION_DOWNLOAD);
        enqueueWork(context1, QuickCopyService.class, DOWNLOAD_JOB_ID, intent);
    ...
    private static void copyFile(InputStream in, OutputStream out,String filename,String category) throws IOException {
        try{
            ...
            boolean iszip = Utils.unpackZip(dirpath+"/QuickBooks/"+category+"/",filename);
            if(iszip){
                File dirFile = new File(context1.getCacheDir(),"QuickBooks/"+category+"/"+filename);
                dirFile.delete();
                ...
                String path = context1.getCacheDir().toString()+"/QuickBooks/"+category+"/"+filename.replace(".zip","");
                Log.e("Files", "Path: " + path);

                String p = path+"/thumbnail.svg";
                Log.e("Files", "thumbnail path......:" + p);
                dataBase.updatequickbooksLocal(p,1,1,filename.replace(".zip",""),category);
            ...
            if(date != null){
                ...
                dataBase.updatequickbooksdownloadstatusfromlocal(1,category,filename.replace(".zip",""));
            ...
            for (filenames in files){
                    ...
                    if (Environment.MEDIA_MOUNTED.equals(state)) {
                        ...
                        for(String filename : files) {
                            ...
                            File folder = new File(context1.getCacheDir() + "/QuickBooks");
                            ...
                            if (success) {
                                ...
                                try {
                                    ...
                                    String category = dataBase.getQuickBooksCategory(filename.replace(".zip",""));
                                    File folder1 = new File(context1.getCacheDir() + "/QuickBooks"+ "/" + category);
                                    if (!folder1.exists()) {
                                        success = folder1.mkdir();
                                    }
                                    out = new FileOutputStream(context1.getCacheDir() + "/QuickBooks" + "/" + category + "/" + filename);
                                    Log.i("WEBVIEW", context1.getCacheDir() + "/QuickBooks" + "/" + filename);
