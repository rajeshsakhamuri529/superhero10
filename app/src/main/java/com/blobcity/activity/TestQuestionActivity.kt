...
class TestQuestionActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (filename in Utils.getListOfFilesFromFolder(localBasic394Path)){
            if (filename.contains("opt")){
                Log.e("original", filename)
                listOfString.add(filename)
            }
            if (filename.contains("question")){
                questionPath = "file:///"+localBasic394Path+filename
            }
        }

        Collections.shuffle(listOfString)
        listOfString.forEachIndexed { index, s ->
            opt1Path = "file:///"+localBasic394Path+listOfString.get(0)
            opt2Path = "file:///"+localBasic394Path+listOfString.get(1)
            opt3Path = "file:///"+localBasic394Path+listOfString.get(2)
            opt4Path = "file:///"+localBasic394Path+listOfString.get(3)
        }
